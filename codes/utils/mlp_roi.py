...
class ResidualBlock(nn.Module):
    def __init__(self, dim=2048, dropout=0.2):
        super().__init__()
...
def main():
    ...
    roi_name = "_".join(opt.roi)
    savedir = f'../../decoded/{opt.subject}/{roi_name}/'
    ...
    for croi in opt.roi:
        if 'conv' in opt.target:
            arr = np.load(f'{mridir}/{opt.subject}_{croi}_betas_ave_tr.npy', mmap_mode='r')
        else:
            arr = np.load(f'{mridir}/{opt.subject}_{croi}_betas_tr.npy', mmap_mode='r')
        X_list.append(arr.astype("float32"))

        arr_te = np.load(f'{mridir}/{opt.subject}_{croi}_betas_ave_te.npy', mmap_mode='r')
    ...
    Y = np.load(f'{featdir}/{opt.subject}_each_{opt.target}_tr.npy', mmap_mode='r')
    Y = Y.reshape([X.shape[0], -1]).astype("float32")

    Y_te = np.load(f'{featdir}/{opt.subject}_ave_{opt.target}_te.npy', mmap_mode='r')
    ...
    model = MLPDecoder(X.shape[1], Y.shape[1], dropout=0.2).to(device)
    ...
    for epoch in range(opt.epochs):
        ...
        if val_loss < best_val:
            best_val = val_loss
            patience_counter = 0
            torch.save(model.state_dict(), f"{savedir}/best_mlp.pt")
        else:
            patience_counter += 1
    ...
    model.load_state_dict(torch.load(f"{savedir}/best_mlp_{roi_name}.pt"))
    ...
    np.save(
        f'{savedir}/{opt.subject}_{roi_name}_scores_{opt.target}_mlp.npy',
        scores
    )
