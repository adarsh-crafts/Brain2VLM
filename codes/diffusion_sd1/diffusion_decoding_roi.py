...
def is_mlp_method(method):
    """Check if method is any variant of mlp (e.g. 'mlp', 'mlp_seed0_d0_w2048_frac1.0')"""
    return method == 'mlp' or method.startswith('mlp_')

def is_cvpr_method(method):
    return method == 'cvpr'

def is_text_method(method):
    return method == 'text'

def is_gan_method(method):
    return method == 'gan'
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="cvpr or text or gan or mlp (or mlp_* variants)",
    )
    parser.add_argument(
    "--roi",
    required=True,
    type=str,
    help="early | lateral | ventral"
    )
    ...
    outdir = f'../../decoded/image-{method}-{opt.roi}/{subject}/'
    ...
    # Load c (Semantics)                                                   #
    # ------------------------------------------------------------------ #
    if is_cvpr_method(method) or is_mlp_method(method):
        roi_c = "ventral"
        if is_mlp_method(method):
            scores_c = np.load(
                f'../../decoded/{subject}/{subject}_{roi_c}_scores_c_mlp.npy')
        else:
            scores_c = np.load(
                f'../../decoded/{subject}/{subject}_{roi_c}_scores_c.npy')
        carr = scores_c[imgidx, :].reshape(77, 768)
        c = torch.Tensor(carr).unsqueeze(0).to('cuda')

    elif is_text_method(method) or is_gan_method(method):
